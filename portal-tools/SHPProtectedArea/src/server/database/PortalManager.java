...
public class PortalManager {
	...
	public int insertGoodRecords(List<Record> recs) {
		...
		if (recs != null && recs.size() > 0) {
			for (Record rec : recs) {
				...
				if (rec.getProtectedArea() == null) {
					query.append(" SET protected_area = null");
				} else {
					query.append(" SET protected_area = (select pa_id from protected_area where name like '"
							+ rec.getProtectedArea() + "' )");
				}
	...
	public int insertGoodRecord(Record rec) {
		...
		if (rec.getProtectedArea() == null) {
			...
			query.append(" SET protected_area = (select pa_id from protected_area where lower(trim(name)) like '"
					+ rec.getProtectedArea().trim().toLowerCase() + "' )");
		}
		query.append(" WHERE id = " + rec.getId());
	...
	public synchronized List<Record> getRecords() {
		rs = DataBaseManager
				.makeQuery(
						"select id, latitude, longitude, nub_concept_id, protected_area from "
								+ ServerConfig.getInstance().dbTableGoods
								+ " where id > "
								+ lastID
								+ " and deleted is null and longitude is not null and latitude is not null and protected_area is null order by id ",
						conx);
		...
		try {
			while (rs.next()) {
				regs.add(new Record(rs.getString("protected_area"), rs
