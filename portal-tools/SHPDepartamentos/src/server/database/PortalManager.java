...
public class PortalManager {
	...
	public int insertGoodRecord(Record rec) {
		...
		if (rec.getDepartamento() == null) {
			...
			query.append(" SET iso_department_code_calculated = (select iso_department_code from department where department_name like ('"
					+ rec.getDepartamento() + "'))");
		}
	...
	public synchronized List<Record> getRecords() {
		rs = DataBaseManager
				.makeQuery(
						"select id,latitude,longitude,nub_concept_id,iso_department_code_calculated from "
		...
		try {
			while (rs.next()) {
				regs.add(new Record(rs
						.getString("iso_department_code_calculated"), rs
