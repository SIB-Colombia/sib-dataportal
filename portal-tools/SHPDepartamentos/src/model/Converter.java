...
public class Converter {
	...
	public static Vector<Double> geographicCoordinatesToUTM(double latitude,
			double longitude) {
		...
		double ni = (c
				/ (1 + exp * Math.pow(Math.pow(Math.cos(laradians), 2), 0.5)) * 0.9996);
		double zeta = (exp / 2) * Math.pow(xi, 2)
				* Math.pow(Math.cos(laradians), 2);
		double eta = Math.atan(Math.tan(laradians) / Math.cos(lambda))
				- laradians;
		...
		double bfi = 0.9996 * c
				* (laradians - (alfa * j_2) + (beta * j_4) - (gamma * j_6));
