...
public class PortalManager {
	...
	public int insertGoodRecord(Record rec) {
		...
		if (rec.getMarineZone() == null) {
			...
			query.append(" SET marine_zone = (select mask from marine_zone where description like ('"
					+ rec.getMarineZone().split(",")[0] + "'))");
		}
	...
	public synchronized List<Record> getRecords() {
		rs = DataBaseManager
				.makeQuery(
						"select id,latitude,longitude,nub_concept_id,marine_zone from "
								+ ServerConfig.getInstance().dbTableGoods
								+ " where id > "
								+ lastID
								+ " and deleted is null and latitude is not null and longitude is not null and marine_zone is null order by id ",
						conx);
		...
		try {
			while (rs.next()) {
				regs.add(new Record(rs.getString("marine_zone"), rs
