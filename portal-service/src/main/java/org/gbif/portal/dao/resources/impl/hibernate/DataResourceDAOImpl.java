...
public class DataResourceDAOImpl extends HibernateDaoSupport implements DataResourceDAO {
  ...
  public List<DataResource> findDataResources(final String nameStub, final boolean fuzzy,
    final DataProvider dataProvider, final BasisOfRecord basisOfRecord, final Date modifiedSince, final int startIndex,
  ...
  public List findDataResourcesAndProvidersAndNetworks(final String nameStub, final boolean fuzzy,
    final boolean anyOccurrence, final boolean includeCountrySearch, final int startIndex, final int maxResults) {
    ...
    try {
      ...
      for (Iterator iter = results.iterator(); iter.hasNext();) {
        ...
        if (result[0].equals("resource")) {
          modelObject =
            new DataResource(id, (String) result[2], ((BigInteger) result[3]).intValue() == 1, (Integer) result[4],
              (Integer) result[5], (Integer) result[6], (Integer) result[7], (Integer) result[9], (String) result[10]);
          ...
          modelObject =
            new ResourceNetwork(id, (String) result[2], (String) result[11], (Integer) result[4], (Integer) result[5],
              (Integer) result[6], (Integer) result[7], (Integer) result[8]);
        }
  ...
  public List<Object[]> getCountryCountsForDataResource(final Long dataResourceId, final boolean geoRefOnly) {
    ...
    return (List) template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        StringBuffer sb =
          new StringBuffer("select c.id as country_id, c.iso_country_code as iso_country_code, c.region as region,");
        if (geoRefOnly) {
  ...
  public List getDataResourceList(final Long dataProviderId) {
    ...
    return (List) template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        Query query =
          session
            .createQuery("select dr.id, dr.name from DataResource dr where dr.dataProvider.id = ? and dr.deleted is null order by dr.name");
  ...
  public List<DataResource> getDataResourcesForProvider(final long dataProviderId) {
    ...
    return (List<DataResource>) template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        Query query =
          session
            .createQuery("from DataResource dr inner join fetch dr.dataProvider dp where dp.id = ? and dr.deleted is null order by dr.name");
        query.setParameter(0, dataProviderId);
        query.setCacheable(true);
        return query.list();
      }
  ...
  public List<DataResource> getDataResourcesForResourceNetwork(final long resourceNetworkId) {
    ...
    return (List<DataResource>) template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        Query query =
          session.createQuery("select nm.dataResource from NetworkMembership nm" + " join nm.resourceNetwork rn"
            + " left join fetch nm.dataResource.dataProvider" + " where rn.id = ? and nm.dataResource.deleted is null"
  ...
  public List<DataResource> getDataResourcesWithNonSharedTaxonomies() {
    ...
    return (List<DataResource>) template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        Query query =
          session.createQuery("from DataResource dr inner join fetch dr.dataProvider"
            + " where dr.sharedTaxonomy=false and dr.conceptCount>0" + " and dr.deleted is null" + " order by dr.name");
        query.setCacheable(true);
        return query.list();
      }
    });
  ...
  public List<Object[]> getDataResourceWithOccurrencesFor(final Long taxonConceptId, final String rank,
    ...
    return (List) template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        StringBuffer sb =
          new StringBuffer("select oc.dataResource.id, oc.dataResource.name, oc.dataProvider.name, count(oc.id) "
  ...
  public List<Character> getDatasetAlphabet() {
    ...
    List<String> dataResourceChars = (List<String>) template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        Query query =
          session
            .createSQLQuery("select distinct(SUBSTRING(display_name,1,1)) from data_resource where display_name is not null and deleted is null order by display_name");
        return query.list();
      }
    });
    List<String> dataProviderChars = (List<String>) template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        Query query =
          session
            .createSQLQuery("select distinct(SUBSTRING(name,1,1)) from data_provider where name is not null order by name");
        return query.list();
      }
    });
    /*
  ...
  public int getTotalDataResourceCount() {
    ...
    Object result = template.execute(new HibernateCallback() {
      ...
      public Object doInHibernate(Session session) {
        Query query =
          session
            .createQuery("select count(dr.id) from DataResource dr where dr.deleted is null and dr.dataProvider.deleted is null"
