...
begin
    INSERT INTO taxon_name(canonical, supra_generic, author, rank) 
		...
		FROM col_families cf left outer JOIN taxon_name tn on canonical = kingdom 
		WHERE kingdom is NOT NULL 
		AND (canonical is null or tn.rank!=1000) 
		and kingdom not in (select canonical from taxon_name tn, col_families ct where canonical = kingdom and tn.rank = 1000)
		group by 1
        union distinct
        SELECT distinct phylum, phylum, null, 2000 
		FROM col_families cf left outer JOIN taxon_name tn on canonical = phylum 
		WHERE phylum is NOT NULL 
		AND (canonical is null or tn.rank!=2000) 
		and phylum not in (select canonical from taxon_name tn, col_families ct where canonical = phylum and tn.rank = 2000)
		group by 1
        union distinct
        SELECT distinct class, class, null, 3000 
		FROM col_families cf left outer JOIN taxon_name tn on canonical = class 
		WHERE class is NOT NULL 
		AND (canonical is null or tn.rank!=3000) 
		and class not in (select canonical from taxon_name tn, col_families ct where canonical = class and tn.rank = 3000)
		group by 1
        union distinct
        SELECT distinct order_rank, order_rank, null, 4000 
		FROM col_families cf left outer JOIN taxon_name tn on canonical = order_rank 
		WHERE order_rank is NOT NULL 
		AND (canonical is null or tn.rank!=4000) 
		and order_rank not in (select canonical from taxon_name tn, col_families ct where canonical = order_rank and tn.rank = 4000)
		group by 1
        union distinct
        SELECT distinct super_family, super_family, null, 4500 
		FROM col_families cf  left outer JOIN taxon_name tn on canonical = super_family 
		WHERE super_family is NOT NULL 
		AND (canonical is null or tn.rank!=4500) 
		and super_family not in (select canonical from taxon_name tn, col_families ct where canonical = super_family and tn.rank = 4500)
		group by 1
        union distinct
        SELECT distinct family, family, null, 5000 
		FROM col_families cf left outer JOIN taxon_name tn on canonical = family 
		WHERE family is NOT NULL 
		AND (canonical is null or tn.rank!=5000) 
		and family not in (select canonical from taxon_name tn, col_families ct where canonical = family and tn.rank = 5000)
		group by 1;
    
    INSERT INTO taxon_name(canonical, generic, author, rank) 
        SELECT distinct genus, genus, null, 6000 
		FROM col_taxa ct left outer JOIN taxon_name tn on canonical = genus 
		WHERE genus is NOT NULL 
		AND (canonical is null or tn.rank!=6000) 
		and genus not in (select canonical from taxon_name tn, col_taxa ct where canonical = genus and tn.rank = 6000)
		group by 1;
        
    INSERT INTO taxon_name(canonical, generic, specific_epithet, author, rank)         
        SELECT scientific_name, genus, species, ct.author, 7000 
		FROM col_taxa ct left outer JOIN taxon_name tn on canonical = scientific_name 
		WHERE species is NOT NULL 
		AND ct.rank='Species' 
		AND (canonical is null or tn.rank!=7000 or ((tn.author is not null and ct.author is not null and tn.author!=ct.author) or (tn.author is null and ct.author is not null) or (ct.author is null and tn.author is not null))) 
		and scientific_name not in (select canonical from taxon_name tn, col_taxa ct where canonical = scientific_name and tn.rank = 7000 and ((tn.author is not null and ct.author is not null and tn.author=ct.author) or (tn.author is null and ct.author is null)))
    ...
    INSERT INTO taxon_name(canonical, generic, specific_epithet, infraspecific, infraspecific_marker, author, rank)         
		...
		FROM col_taxa ct left outer JOIN taxon_name tn on canonical = scientific_name 
		WHERE infraspecies is NOT NULL AND ct.rank='Infraspecies' 
		AND (canonical is null or tn.rank!=8000 or ((tn.author is not null and ct.author is not null and tn.author!=ct.author) or (tn.author is null and ct.author is not null) or (ct.author is null and tn.author is not null))) 
		and scientific_name not in (select canonical from taxon_name tn, col_taxa ct where canonical = scientific_name and tn.rank = 8000 and ((tn.author is not null and ct.author is not null and tn.author=ct.author) or (tn.author is null and ct.author is null)))
		group by scientific_name, author;
end;
|
delimiter ;
