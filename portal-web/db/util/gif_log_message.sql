...
BLOCK1: BEGIN
	...
	LOOP1:LOOP
		...
		insert into gbif_log_message(portal_instance_id, log_group_id, event_id, level, data_provider_id, data_resource_id, occurrence_id, taxon_concept_id, message, restricted, count, timestamp)
		select 1 , logGroup, 1008, 40000,  data_provider_id, data_resource_id, id, taxon_concept_id , concat(iso_department_code , " (Lat: " , latitude , ", Lon: " , longitude , " ): Geospatial issues: coordinates fall outside specified department, department calculated: " , iso_department_code_calculated ), 0, 1, NOW() from occurrence_record where iso_department_code_calculated is not null and iso_department_code != iso_department_code_calculated;
	end loop LOOP1;
end BLOCK1
//
DELIMITER ;
