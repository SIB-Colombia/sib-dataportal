...
-->
<c:if test="${latitude!=null}">${latitude>0?'+':'-'}<gbif:decimal noDecimalPlaces="4">${latitude<0 ? (latitude*-1): latitude}</gbif:decimal></c:if>${latitude!=null && longitude!=null ?', ':''} 
<c:if test="${longitude!=null}">${longitude>0?'+':'-'}<gbif:decimal noDecimalPlaces="4">${longitude<0 ? (longitude*-1): longitude}</gbif:decimal></c:if>
