...
<c:choose>
	<c:when test="${dataProvider.occurrenceCount>0}">
		...
		<ul class="genericList">
			<li>
				<a href="${pageContext.request.contextPath}/publicadores/provider/${dataProvider.key}/logs/"><spring:message code="view.event.logs.for" text="View event logs for"/> ${dataProvider.name}</a>
			</li>
			<li>
				<a href="${pageContext.request.contextPath}/publicadores/provider/${dataProvider.key}/indexing/"><spring:message code="view.indexing.history.for" text="View indexing history for"/> ${dataProvider.name}</a>
			</li>
		</ul>
	</c:when>
	<c:otherwise>
