...
<c:set var="entityLink" scope="request">publicadores/provider/${dataProvider.key}</c:set>
<c:set var="mapDescription" scope="request">
<c:if test="${param['size']!='small'}">
Specimen and observational data	published by 
</c:if>
<span class="subject"><a href="http://${header.host}${pageContext.request.contextPath}/publicadores/provider/${dataProvider.key}" target="_top">${dataProvider.name}</a></span><br/>
