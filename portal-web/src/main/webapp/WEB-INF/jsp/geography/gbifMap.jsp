...
<c:set var="zoom" scope="request">${zoom==null ? 1 : zoom}</c:set>
	...
	<tr class="firstRow">
		...
		<td class="map" colspan="3" rowspan="3" align="center" valign="middle">
			...
			<img id="overviewMap" 
					...
					<c:if test="${zoom==1}">
					src="${geoServerUrl}?request=GetMap&bgcolor=0x80cedd&styles=,,&layers=gbif:country_fill,gbif:tabDensityLayer,gbif:country_borders&srs=EPSG:4326&filter=()(<Filter><PropertyIsEqualTo><PropertyName>url</PropertyName><Literal><![CDATA[${mapLayerUrl}]]></Literal></PropertyIsEqualTo></Filter>)()&width=${mapWidth}&height=${mapHeight}&Format=image/png&bbox=${minMapLong},${minMapLat},${maxMapLong},${maxMapLat}"
					</c:if>
					<%-- Ignoring bluemarble layer
					<c:if test="${zoom>1 && zoom<6}">
					src="${geoServerUrl}?request=GetMap&bgcolor=0x666698&styles=,,,&layers=gbif:country_fill,gbif:tabDensityLayer,gbif:country_borders,gbif:country_names&srs=EPSG:4326&filter=()(<Filter><PropertyIsEqualTo><PropertyName>url</PropertyName><Literal><![CDATA[${mapLayerUrl}]]></Literal></PropertyIsEqualTo></Filter>)()()&width=${mapWidth}&height=${mapHeight}&Format=image/png&bbox=${minMapLong},${minMapLat},${maxMapLong},${maxMapLat}"
					</c:if>		
					<c:if test="${zoom==6}">
					src="${geoServerUrl}?request=GetMap&bgcolor=0x666698&styles=,,country_borders_black,country_names_blue&layers=bluemarble,gbif:tabDensityLayer,gbif:country_borders,gbif:country_names&srs=EPSG:4326&filter=()(<Filter><PropertyIsEqualTo><PropertyName>url</PropertyName><Literal><![CDATA[${mapLayerUrl}]]></Literal></PropertyIsEqualTo></Filter>)()()&width=${mapWidth}&height=${mapHeight}&Format=image/png&bbox=${minMapLong},${minMapLat},${maxMapLong},${maxMapLat}"
					</c:if>	 --%> 
					<c:if test="${zoom>1 && zoom<=6}">
					src="${geoServerUrl}?request=GetMap&bgcolor=0x80cedd&styles=,,,&layers=gbif:country_fill,gbif:tabDensityLayer,gbif:country_borders,gbif:country_names&srs=EPSG:4326&filter=()(<Filter><PropertyIsEqualTo><PropertyName>url</PropertyName><Literal><![CDATA[${mapLayerUrl}]]></Literal></PropertyIsEqualTo></Filter>)()()&width=${mapWidth}&height=${mapHeight}&Format=image/png&bbox=${minMapLong},${minMapLat},${maxMapLong},${maxMapLat}"
					</c:if>
        ...
        <table cellspacing="5" class="mapKey">
				<c:choose>
					<c:when test='${zoom == 6}'>
