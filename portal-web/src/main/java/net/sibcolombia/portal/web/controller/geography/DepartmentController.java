	...
	public ModelAndView handleRequest(Map<String, String> properties, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(StringUtils.isNotEmpty(departmentKey)){
			...
			if(department!=null){
				...
				List<CountDTO> nonDepartmentCounts = departmentManager.getDepartmentCountsForDepartement(department.getDepartmentCode(), false, locale);
				...
				if(!MapContentProvider.zoomLevelSpecified(request) && !showHosted && department.getMinLongitude()!=null 
						&& department.getMinLatitude()!=null && department.getMaxLongitude()!=null && department.getMaxLatitude()!=null){
					//zoom to the correct level for this department
					mapContentProvider.addMapContent(request, entityType.getName(), department.getKey(), department.getMinLongitude(), 
							department.getMinLatitude(), department.getMaxLongitude(), department.getMaxLatitude());
					mapContentProvider.addPointsTotalsToRequest(request, entityType, department.getKey(), 
							new BoundingBoxDTO(department.getMinLongitude(), department.getMinLatitude(), department.getMaxLongitude(), department.getMaxLatitude()));
				} else {
