 ...
 * Class modified to include all list support
 * SIB Colombia
 ***************************************************************************/
package net.sibcolombia.portal.web.tag;
...
public class AlphabetLinkTag extends TagSupport {
  ...
  private final String[] accepted = {"A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O", "P",
    ...
    for (Character letter : letters) {
      if (!ignoreChars.contains(letter) && acceptedChars.contains(letter.toString())) {
        ...
        if (!(letter == '0')) {
          ...
          if (selected != letter) {
            sb.append('>');
            addLink(sb, contextPath, '0', messageSource.getMessage("dataset.list.showall", null, "Todas", locale));
          } else {
            sb.append(" id=\"chosen\">");
