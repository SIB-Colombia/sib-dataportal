...
public class OccurrenceParameters extends Parameters {
  ...
  private static final String SUBJECT_OCCURRENCEMODIFICATIONDATE =
    "SERVICE.OCCURRENCE.QUERY.SUBJECT.OCCURRENCEMODIFICATIONDATE";
  ...
  public OccurrenceParameters(List<PropertyStoreTripletDTO> triplets, String formatName, String urlBase,
    String servletPath, PathMapping mapping, boolean throwExceptions) throws GbifWebServiceException {
    portalRoot = urlBase;
    ...
    for (PropertyStoreTripletDTO triplet : triplets) {
      ...
      if (triplet.getSubject().equals(SUBJECT_BASISOFRECORD) && triplet.getPredicate().equals(PREDICATE_EQUAL)) {
        basisOfRecordCodes =
          addValue(basisOfRecordCodes, BasisOfRecord.getBasisOfRecord((Integer) triplet.getObject()).getName());
        processed = true;
      } else if ((triplet.getSubject().equals(SUBJECT_KINGDOMID) || triplet.getSubject().equals(SUBJECT_PHYLUMID)
        || triplet.getSubject().equals(SUBJECT_CLASSID) || triplet.getSubject().equals(SUBJECT_ORDERID)
        || triplet.getSubject().equals(SUBJECT_FAMILYID) || triplet.getSubject().equals(SUBJECT_GENUSID)
        || triplet.getSubject().equals(SUBJECT_SPECIESID) || triplet.getSubject().equals(SUBJECT_TAXONCONCEPTID))
        ...
        processed = true;
      } else if (triplet.getSubject().equals(SUBJECT_RESOURCENETWORKID)
        && triplet.getPredicate().equals(PREDICATE_EQUAL)) {
        ...
        processed = true;
      } else if (triplet.getSubject().equals(SUBJECT_ISODEPARTMENTCODE)
        && triplet.getPredicate().equals(PREDICATE_EQUAL)) {
        originIsoDepartmentCodes = addValue(originIsoDepartmentCodes, (String) triplet.getObject());
        processed = true;
      }else if (triplet.getSubject().equals(SUBJECT_ISOCOUNTYCODE)
    	&& triplet.getPredicate().equals(PREDICATE_EQUAL)) {
        ...
        processed = true;
      }else if (triplet.getSubject().equals(SUBJECT_PROTECTEDAREAID)
    	    	&& triplet.getPredicate().equals(PREDICATE_EQUAL)) {
      ...
      } else if (triplet.getSubject().equals(SUBJECT_OCCURRENCEMODIFICATIONDATE)
        && triplet.getPredicate().equals(PREDICATE_GE)) {
        Date date = (Date) triplet.getObject();
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
        calendar.set(Calendar.HOUR, 0);
        ...
        processed = true;
      } else if (triplet.getSubject().equals(SUBJECT_GEOSPATIALISSUES)
        && triplet.getPredicate().equals(PREDICATE_EQUAL)) {
      ...
      if (!processed) {
        String errorString =
          "Could not process triplet: [" + triplet.getSubject() + "],[" + triplet.getPredicate() + "],["
            + triplet.getObject() + "]";
        if (throwExceptions) {
  ...
  public OccurrenceParameters(Map<String, Object> params, PathMapping pathMapping) throws GbifWebServiceException {
        ...
        if (k.equals(KEY_MODE)) {
          ...
          for (String name : scientificNames) {
            ...
            if (wildcardIndex > 0 && wildcardIndex < MINIMUM_WILDCARD_INDEX) {
              throw new GbifWebServiceException("Scientific name string must include at least "
                + MINIMUM_WILDCARD_INDEX + " characters before the first wildcard character");
            }
          }
   ...
   * @return
   */
  private Integer[] getIntegerValue(Map<String, Object> map, String key, String string, Integer minValue,
    Integer maxValue) {
    ...
    if (originalValues == null) {
      ...
      try {
        ...
        if ((minValue != null && values[0] < minValue) || (maxValue != null && values[0] > maxValue)) {
      ...
      for (String originalValue : originalValues) {
        try {
          values[processed] = new Integer(originalValue);
          if ((minValue != null && values[processed] < minValue) || (maxValue != null && values[processed] > maxValue)) {
            log.error("Integer value " + originalValue + " out of range (" + minValue + " - " + maxValue + ")");
          } else {
            processed++;
          }
        } catch (Exception e) {
          log.error("Invalid integer for " + key + ": " + originalValue);
        }
      }
      if (processed == 0) {
        values = null;
  ...
  /**
   * @return the originIsoCountyCode
   */
  public String[] getOriginIsoCountyCodes() {
    return originIsoCountyCodes;
  }
  ...
  /**
   * @return the protectedAreaIds
   */
  public String[] getProtectedAreaIds() {
    ...
    if (rt != Action.SCHEMA && rt != Action.STYLESHEET) {
      if (requestType == Action.COUNT || requestType == Action.LIST) {
        if (taxonConceptKeys != null)
          map.put(KEY_TAXONCONCEPTKEY, taxonConceptKeys);
        if (scientificNames != null)
          map.put(KEY_SCIENTIFICNAME, scientificNames);
        if (dataProviderKeys != null)
          map.put(KEY_DATAPROVIDERKEY, dataProviderKeys);
        if (dataResourceKeys != null)
          map.put(KEY_DATARESOURCEKEY, dataResourceKeys);
        if (resourceNetworkKeys != null)
          map.put(KEY_RESOURCENETWORKKEY, resourceNetworkKeys);
        if (basisOfRecordCodes != null)
          map.put(KEY_BASISOFRECORDCODE, basisOfRecordCodes);
        if (hostIsoCountryCodes != null)
          map.put(KEY_HOSTISOCOUNTRYCODE, hostIsoCountryCodes);
        if (originIsoCountryCodes != null)
          map.put(KEY_ORIGINISOCOUNTRYCODE, originIsoCountryCodes);
        if (originIsoDepartmentCodes != null)
        ...
        if (cellIds != null)
          map.put(KEY_CELLID, cellIds);
        if (centiCellIds != null)
          map.put(KEY_CENTICELLID, centiCellIds);
        if (boundingBox.getLower() != null)
          map.put(KEY_MINLATITUDE, boundingBox.getLower().toString());
        if (boundingBox.getUpper() != null)
          map.put(KEY_MAXLATITUDE, boundingBox.getUpper().toString());
        if (boundingBox.getLeft() != null)
          map.put(KEY_MINLONGITUDE, boundingBox.getLeft().toString());
        if (boundingBox.getRight() != null)
          map.put(KEY_MAXLONGITUDE, boundingBox.getRight().toString());
        if (timePeriod.getStartPeriod() != null)
          map.put(KEY_STARTDATE, formatDate(timePeriod.getStartPeriod()));
        if (timePeriod.getEndPeriod() != null)
          map.put(KEY_ENDDATE, formatDate(timePeriod.getEndPeriod()));
        if (startYear != null)
          map.put(KEY_STARTYEAR, startYear.toString());
        if (endYear != null)
          map.put(KEY_ENDYEAR, endYear.toString());
        if (years != null)
          map.put(KEY_YEAR, getStringArray(years));
        if (months != null)
          map.put(KEY_MONTH, getStringArray(months));
        if (days != null)
          map.put(KEY_DAY, getStringArray(days));
        if (coordinateStatus != null)
          map.put(KEY_COORDINATESTATUS, coordinateStatus.toString());
        if (coordinateIssues != null)
          map.put(KEY_COORDINATEISSUES, coordinateIssues.toString());
        if (typesOnly)
          map.put(KEY_TYPESONLY, "true");
        if (minAltitude != null)
          map.put(KEY_MINALTITUDE, minAltitude);
        if (maxAltitude != null)
          map.put(KEY_MAXALTITUDE, maxAltitude);
        if (minDepth != null)
          map.put(KEY_MINDEPTH, minDepth);
        if (maxDepth != null)
          map.put(KEY_MAXDEPTH, maxDepth);
        // support institutionCode, collectionCode, catalogueNumber
        if (institutionCodes != null)
          map.put(KEY_INSTITUTIONCODE, institutionCodes);
        if (collectionCodes != null)
          map.put(KEY_COLLECTIONCODE, collectionCodes);
        if (catalogueNumbers != null)
          map.put(KEY_CATALOGUENUMBER, catalogueNumbers);
      }
  ...
  public List<PropertyStoreTripletDTO> getTriplets(TaxonomyManager taxonomyManager) {
    ...
    if (dataResourceKeys != null) {
      for (String dataResourceKey : dataResourceKeys) {
        addTriplet(triplets, SUBJECT_DATARESOURCEID, PREDICATE_EQUAL, new Long(dataResourceKey));
      }
    }
    if (dataProviderKeys != null) {
      for (String dataProviderKey : dataProviderKeys) {
        addTriplet(triplets, SUBJECT_DATAPROVIDERID, PREDICATE_EQUAL, new Long(dataProviderKey));
      }
    }
    if (resourceNetworkKeys != null) {
      for (String resourceNetworkKey : resourceNetworkKeys) {
        addTriplet(triplets, SUBJECT_RESOURCENETWORKID, PREDICATE_EQUAL, new Long(resourceNetworkKey));
      }
    }
    if (taxonConceptKeys != null) {
      for (String taxonConceptKey : taxonConceptKeys) {
        try {
          BriefTaxonConceptDTO taxon = taxonomyManager.getTaxonConceptFor(taxonConceptKey);
          ...
          if (taxon != null) {
            ...
            switch (taxon.getRankValue()) {
              case 1000:
                subject = SUBJECT_KINGDOMID;
                break;
              case 2000:
                subject = SUBJECT_PHYLUMID;
                break;
              case 3000:
                subject = SUBJECT_CLASSID;
                break;
              case 4000:
                subject = SUBJECT_ORDERID;
                break;
              case 5000:
                subject = SUBJECT_FAMILYID;
                break;
              case 6000:
                subject = SUBJECT_GENUSID;
                break;
              case 7000:
                subject = SUBJECT_SPECIESID;
                break;
            }
            addTriplet(triplets, subject, PREDICATE_EQUAL, new Long(taxon.getKey()));
          }
        } catch (Exception e) {
          log.error("Failed to get taxon " + taxonConceptKey);
        }
      }
    }
    if (scientificNames != null) {
      for (String scientificName : scientificNames) {
        String name = scientificName.replace("*", "%");
        String predicate = (name.indexOf("%") >= 0) ? PREDICATE_LIKE : PREDICATE_EQUAL;
        addTriplet(triplets, SUBJECT_SCIENTIFICNAME, predicate, name);
      }
    }
    if (hostIsoCountryCodes != null) {
      for (String hostIsoCountryCode : hostIsoCountryCodes) {
        addTriplet(triplets, SUBJECT_HOSTCOUNTRYCODE, PREDICATE_EQUAL, hostIsoCountryCode);
      }
    }
    if (originIsoCountryCodes != null) {
      for (String originIsoCountryCode : originIsoCountryCodes) {
        addTriplet(triplets, SUBJECT_ISOCOUNTRYCODE, PREDICATE_EQUAL, originIsoCountryCode);
      }
    }
    if (originIsoDepartmentCodes != null) {
      for (String originIsoDepartmentCode : originIsoDepartmentCodes) {
        addTriplet(triplets, SUBJECT_ISODEPARTMENTCODE, PREDICATE_EQUAL, originIsoDepartmentCode);
      }
    }
    if (originIsoCountyCodes != null) {
    ...
    if (originRegionCodes != null) {
      for (String originRegionCode : originRegionCodes) {
        addTriplet(triplets, SUBJECT_REGIONCODE, PREDICATE_EQUAL, originRegionCode);
      }
    }
    if (basisOfRecordCodes != null) {
      for (String basisOfRecordCode : basisOfRecordCodes) {
        addTriplet(triplets, SUBJECT_BASISOFRECORD, PREDICATE_EQUAL, BasisOfRecord.getBasisOfRecord(basisOfRecordCode)
          .getValue());
      }
    }
    // support institutionCode, collectionCode, catalogueNumber
    if (institutionCodes != null) {
      for (String institutionCode : institutionCodes) {
        addTriplet(triplets, SUBJECT_INSTITUTIONCODE, PREDICATE_EQUAL, institutionCode);
      }
    }
    if (collectionCodes != null) {
      for (String collectionCode : collectionCodes) {
        addTriplet(triplets, SUBJECT_COLLECTIONCODE, PREDICATE_EQUAL, collectionCode);
      }
    }
    if (catalogueNumbers != null) {
      for (String catalogueNumber : catalogueNumbers) {
        addTriplet(triplets, SUBJECT_CATALOGUENUMBER, PREDICATE_EQUAL, catalogueNumber);
      }
    }

    if (cellIds != null) {
      for (String cellId : cellIds) {
        addTriplet(triplets, SUBJECT_CELLID, PREDICATE_EQUAL, new Integer(cellId));
      }
    }
    if (centiCellIds != null) {
      for (String centiCellId : centiCellIds) {
        addTriplet(triplets, SUBJECT_CENTICELLID, PREDICATE_EQUAL, new Integer(centiCellId));
      }
    }
    if (boundingBox.getLeft() != null && boundingBox.getLower() != null && boundingBox.getRight() != null
      && boundingBox.getUpper() != null) {
      List<PropertyStoreTripletDTO> cellIdTriplets =
        BoundingBoxUtils.getTripletsFromLatLongBoundingBox(
          TRIPLET_NAMESPACE,
          new LatLongBoundingBox(boundingBox.getLeft(), boundingBox.getLower(), boundingBox.getRight(), boundingBox
            .getUpper()));
    ...
    if (years != null) {
      for (Integer year : years) {
        addTriplet(triplets, SUBJECT_YEAR, PREDICATE_EQUAL, year);
      }
    }
    if (months != null) {
      for (Integer month : months) {
        addTriplet(triplets, SUBJECT_MONTH, PREDICATE_EQUAL, month);
      }
    }
    if (days != null) {
      for (Integer day : days) {
        addTriplet(triplets, SUBJECT_DAY, PREDICATE_EQUAL, day);
      }
    }
    ...
    if (coordinateIssues != null) {
      addTriplet(triplets, SUBJECT_GEOSPATIALISSUES, coordinateIssues ? PREDICATE_NEQUAL : PREDICATE_EQUAL,
        new Integer(0));
    }
    if (typesOnly) {
