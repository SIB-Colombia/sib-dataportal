...
public class SmallTaxonomyBrowserTag extends TagSupport {
   ...
   * @throws JspException
   */
  void addConceptsRecursively(List<BriefTaxonConceptDTO> conceptList, StringBuffer sb, String contextPath)
    throws JspException {
    ...
    // if both null, then they are both root concepts
      (nextConcept.getParentConceptKey() == null && currentParentKey == null) || (nextConcept.getParentConceptKey()
        .equals(currentParentKey)))) {
      // add all elements to list div
      sb.append("<div class=\"list\">");
      while (currentConcept != null && (
      // if both null, then they are both root concepts
        (currentConcept.getParentConceptKey() == null && currentParentKey == null) || (currentConcept
          .getParentConceptKey().equals(currentParentKey)))) {
        // render the concept
...
// sb.append(StringUtils.capitalize(concept.getRank()));
    ...
    if (addOverviewLink) {
      ...
      if (conceptKey != null
        && (TaxonRankType.isRecognisedMajorRank(concept.getRank()) || TaxonRankType.SUB_SPECIES_STR.equals(concept
          .getRank()))) {
        ...
        if (concept.getRank().equals(TaxonRankType.SPECIES_STR)
          ...
          try {
            occurrencesCount =
              occurrenceManager.countOccurrenceRecords(null, null, null, concept.getKey(), null, null, null, null,null, null, null,
          ...
          sb.append((occurrencesCount == 0 ? messageSource.getMessage(noOccurrences, null, locale) : messageSource
            .getMessage(numberOfOccurrences, messageArgs, locale)));
          sb.append("</span>");
        }
      }
    }
  ...
  protected String getLink(String contextPath, SpecialTreeNode specialTreeNode) {
    ...
    if (specialTreeNode.isHigherTaxa()) {
      if (specialTreeNode.getParentConceptKey() != null)
        return getLink(contextPath, specialTreeNode.getParentConceptKey()) + "?"
          + specialTreeNode.getExpandRequestParameter() + "=true#" + specialTreeNode.getExpandRequestParameter();
    } else if (specialTreeNode.isShowChildren()) {
      return "#" + specialTreeNode.getExpandRequestParameter();
    } else if (selectedConcept != null) {
      return getLink(contextPath, selectedConcept) + "?" + specialTreeNode.getExpandRequestParameter() + "=true#"
        + specialTreeNode.getExpandRequestParameter();
    } else {
      return getLink(contextPath, specialTreeNode.getParentConceptKey()) + "?"
        + specialTreeNode.getExpandRequestParameter() + "=true#" + specialTreeNode.getExpandRequestParameter();
    }
