...
public class TaxonResolvingController extends RestController {
   ...
   * @throws Exception
   */
  public void addActions(TaxonConceptDTO taxonConcept, HttpServletRequest request, HttpServletResponse response)
    throws Exception {
   ...
   * @throws Exception
   */
  public void addImages(TaxonConceptDTO taxonConcept, HttpServletRequest request, HttpServletResponse response)
    throws Exception {
    SearchResultsDTO imageResults =
      taxonomyManager.findImagesFor(taxonConcept.getKey(), new SearchConstraints(0, maxImagesToShow));
  ...
  protected void addKeywords(TaxonConceptDTO taxonConcept, HttpServletRequest request, HttpServletResponse response) {
    ...
    if (partners != null && !partners.isEmpty()) {
      for (PartnerConceptBean partner : partners) {
        ...
        // add common names
        List<Map<String, List<CommonNameDTO>>> commonNamesMapList = partner.getCommonNames();
   ...
   * @throws Exception
   */
  public void addNames(TaxonConceptDTO taxonConcept, HttpServletRequest request, HttpServletResponse response)
    throws Exception {
    // retrieve full classification
    List<BriefTaxonConceptDTO> fullClassification =
      taxonomyManager.getClassificationFor(taxonConcept.getKey(), false, null, allowUnconfirmed);
   ...
   * @throws Exception
   */
  public void addOccurrences(TaxonConceptDTO taxonConcept, HttpServletRequest request, HttpServletResponse response)
    throws Exception {
    ...
    // Total occurrence count - only doing this for species and subspecies - a count against a higher rank will be very
// expensive
    // TODO Ideally this would hit a statistics table.
    if (TaxonRankType.isGenusOrLowerRank(taxonConcept.getRank())) {
      occurrenceCount =
        occurrenceManager.countOccurrenceRecords(null, null, null, taxonConcept.getKey(), null, null, null, null, null, null,null,null,null,null,null, null,
      ...
      List<CountDTO> resourceCounts =
        dataResourceManager.getDataResourceWithOccurrencesFor(taxonConcept.getKey(), taxonConcept.getRank(), true);
    ...
    if (taxonConcept.getRank().equals(TaxonRankType.SPECIES.getName())) {
      ...
      int subspecies =
        taxonomyManager.countChildConceptsFor(taxonConcept.getKey(), TaxonRankType.SUB_SPECIES, false, true, false);
      request.setAttribute("subspeciesCount", subspecies);
    } else if (taxonConcept.getRank().equals(TaxonRankType.GENUS.getName())) {
      // add a species count
      int species =
        taxonomyManager.countChildConceptsFor(taxonConcept.getKey(), TaxonRankType.SPECIES, false, true, false);
      ...
      // add a genera count
      int genera =
        taxonomyManager.countChildConceptsFor(taxonConcept.getKey(), TaxonRankType.GENUS, false, true, false);
    ...
    if (showCharts && occurrenceCount != -1 && pointsTotal != null && occurrenceCount != 0) {
      ...
      legend.put(messageSource.getMessage("charts.georeferenced.with.coordinates", null, "With coordinates", locale),
        pointsTotal.doubleValue());
      legend.put(
        messageSource.getMessage("charts.georeferenced.without.coordinates", null, "Without coordinates", locale),
        (double) (occurrenceCount - pointsTotal.intValue()));
...
// countryLegend.put(messageSource.getMessage("country."+countDTO.getKey(), null, countDTO.getKey(), locale),
// countDTO.getCount().doubleValue());
// }
...
//
      ...
      for (int i = 0; i < 5 && iter.hasNext(); i++) {
        ...
        regionLegend.put(messageSource.getMessage("region." + countDTO.getKey(), null, countDTO.getKey(), locale),
          countDTO.getCount().doubleValue());
      }
      // add the remainder together - Other
...
// request.setAttribute("regionChart", chartFileName);
      ...
      request.setAttribute("nonGeoreferencedPercentage",
        100 - (int) (((float) pointsTotal / (float) occurrenceCount) * 100));
    }
  ...
  @SuppressWarnings({"unchecked"})
  public void
    addPartnerConcepts(TaxonConceptDTO taxonConcept, HttpServletRequest request, HttpServletResponse response)
      throws Exception {
    List<TaxonConceptDTO> partnerConcepts =
      taxonomyManager.getAuthoritativeTaxonConceptsForNubTaxonConcept(taxonConcept.getKey());
    ...
    for (TaxonConceptDTO partnerConcept : partnerConcepts) {
      ...
      partner.setHigherConcepts(taxonomyManager.getClassificationFor(partnerConcept.getKey(), false, null,
        allowUnconfirmed));
      ...
      List<RelationshipAssertionDTO> relationshipAssertions =
        taxonomyManager.findRelationshipAssertionsForFromTaxonConcept(partnerConcept.getKey());
      partner.setRelationshipAssertions(relationshipAssertions);

      if (logger.isDebugEnabled())
        logger.debug("Added partner: " + partnerConcept.getTaxonName() + " from " + partner.getDataResource().getName()
          + " (" + partner.getDataResource().getBasisOfRecord() + ")");
    }
  }
   ...
   * @throws Exception
   */
  public void addTypifications(TaxonConceptDTO taxonConcept, HttpServletRequest request, HttpServletResponse response)
    throws Exception {
    if (taxonConcept.getRankValue() >= TaxonRank.FAMILY.getValue()) {
      request.setAttribute("typifications",
        taxonomyManager.getTypificationRecordsForTaxonConcept(taxonConcept.getKey()));
    }
  }
   ...
   * @throws Exception
   */
  public void addWarnings(TaxonConceptDTO taxonConcept, HttpServletRequest request, HttpServletResponse response)
    throws Exception {
    ...
    SearchResultsDTO searchResults =
      taxonomyManager.findTaxonConceptsWithSameScientificNameAndRankAs(taxonConcept.getKey(), null,
        taxonConcept.getDataResourceKey(), new SearchConstraints(0, 10));
    if (!searchResults.isEmpty()) {
      ...
      for (BriefTaxonConceptDTO taxonConceptDTO : results) {
        List<BriefTaxonConceptDTO> classification =
          taxonomyManager.getClassificationFor(taxonConceptDTO.getKey(), false, null, allowUnconfirmed);
   ...
   */
  @SuppressWarnings("unchecked")
  private List<Map<String, List<CommonNameDTO>>> getOrderedCommonNames(HttpServletRequest request,
    TaxonConceptDTO partnerConcept) throws ServiceException {
    // group the names
    List<CommonNameDTO> commonNames =
      taxonomyManager.findCommonNamesForTaxonConcept(partnerConcept.getKey(), new SearchConstraints(0,
        maxCommonNamesToDisplay));
    ...
    // sort them
    Collections.sort(orderedCommonNames, new Comparator() {
  ...
  public ModelAndView handleRequest(Map<String, String> propertiesMap, HttpServletRequest request,
    ...
    logger.info("conceptIdentifier:" + conceptIdentifier);
    logger.info("dataResourceKey:" + dataResourceKey);
    ...
    if (dataResourceKey != null || dataProviderKey != null) {
      ...
      if (taxonConceptDTO != null && !taxonConceptDTO.getIsNubConcept()
        && taxonConceptDTO.getPartnerConceptKey() != null) {
      ...
      taxonConceptDTO =
        taxonomyManager.getTaxonConceptFor(conceptIdentifier, RequestContextUtils.getLocale(request).getLanguage());
      // redirect to nub concept if this is not a nub concept and if available
      if (taxonConceptDTO != null && !taxonConceptDTO.getIsNubConcept()
        && taxonConceptDTO.getPartnerConceptKey() != null) {
        logger.debug("Redirecting to nub concept page");
        return new ModelAndView(new RedirectView(request.getContextPath() + "/" + urlRoot + "/"
          + taxonConceptDTO.getPartnerConceptKey()));
      }
    } else {
    ...
    if (nubDataProvider != null)
      searchResultsDTO =
        taxonomyManager.findTaxonConcepts(conceptIdentifier, false, rank, nubDataProvider.getKey(), null, null, null,
          RequestContextUtils.getLocale(request).getLanguage(), null, allowUnconfirmedNames, false, searchConstraints);
    ...
    if (searchResultsDTO == null || searchResultsDTO.getResults().size() == 0) {
      ...
      searchResultsDTO =
        taxonomyManager.findTaxonConcepts(conceptIdentifier, false, rank, null, null, null, null, RequestContextUtils
          .getLocale(request).getLanguage(), null, allowUnconfirmedNames, false, searchConstraints);
    }
    List<TaxonConceptDTO> taxonConcepts = searchResultsDTO.getResults();
    // if one concept returned, use this concept
    if (taxonConcepts.size() == 1) {
      taxonConceptDTO = taxonConcepts.get(0);
   ...
   * @return
   */
  protected ModelAndView populateView(String taxonConceptKey, Map<String, String> propertiesMap,
    ...
    logger.info("Retrieving full taxon concept dto");
    ...
    logger.info("Adding partner concepts");
    addPartnerConcepts(taxonConcept, request, response);
    logger.info("Adding typifications");
