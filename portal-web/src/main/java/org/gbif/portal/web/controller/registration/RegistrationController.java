...
// providerList.removeAll(providerRegistrationLogins);
  ...
  public ModelAndView completeManualRegistration(HttpServletRequest request, HttpServletResponse response)
    ...
    if (result.hasErrors()) {
      ...
      return new ModelAndView(new RedirectView(request.getContextPath() + "/register/showDataResources?businessKey="
        + businessKey));
    }
  }
    ...
    if (resourceType2namespaceList != null) {
      for (String resourceType : resourceType2namespaceList.keySet()) {
        ...
        try {
          ...
          Processor workflow =
            (Processor) propertyStore.getProperty(namespace, "REGISTRATION.METADATA.WORKFLOW", Processor.class);
...
// }
  ...
  protected void initProviderBinder(HttpServletRequest request, ServletRequestDataBinder binder) throws Exception {
    ...
    if (maxSecondaryIndex + 1 > currentSize) {
      ...
      if (isFormSubmission(request)) {
        ...
        providerDetail.setBusinessSecondaryContacts(providerDetail.getBusinessSecondaryContacts().subList(0,
          maxSecondaryIndex + 1));
      }
    }
    ...
    if (maxResourceIndex + 1 > currentSize) {
      ...
      if (isFormSubmission(request)) {
        logger.info("shrinking");
        providerDetail.setBusinessResources(providerDetail.getBusinessResources().subList(0, maxResourceIndex + 1));
      }
    }
  }
  ...
  public ModelAndView newUser(HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    verificationMessage.setText("Please visit the following link to confirm your e-mail address:\n\n" + "http://"
      + request.getHeader("host") + request.getContextPath() + "/user/verification" + "?fn=" + ul.getFirstName()
      + "&sn=" + ul.getSurname() + "&e=" + ul.getEmail() + "&u=" + ul.getUsername() + "&p=" + encryptedPassword);
    try {
      mailSender.send(verificationMessage);
    } catch (MailException e) {
      // simply log it and go on...
  ...
  protected Map<String, Object> referenceDataForProvider(HttpServletRequest request) throws Exception {
    ...
    data.put(RegistrationController.REQUEST_COUNTRIES, geospatialManager.findAllCountries(request.getLocale())
      .getResults());
    return data;
  }
   ...
   */
  protected Map<String, Object> referenceDataForResource(HttpServletRequest request, ResourceDetail resourceDetail)
    throws Exception {
    Map<String, Object> data = new HashMap<String, Object>();
    ...
    if (StringUtils.isNotEmpty(resourceDetail.getHighestTaxaConceptId())) {
      data.put(RegistrationController.REQUEST_CONCEPTS,
        taxonomyManager.getClassificationFor(resourceDetail.getHighestTaxaConceptId(), true, null, true));
   ...
   * Populates the ReferenceData for the resource list page
   */
  protected Map<String, Object> referenceDataForResourceList(HttpServletRequest request, ProviderDetail providerDetail)
    throws Exception {
    Map<String, Object> data = new HashMap<String, Object>();
    Locale locale = RequestContextUtils.getLocale(request);
    DataProviderDTO nubProvider = dataResourceManager.getNubDataProvider();
...
// detail.getBusinessPrimaryContact().setEmail(namePhoneEmail[2]);
  ...
  public ModelAndView updateRegistrationLogins(HttpServletRequest request, HttpServletResponse response)
    ...
    if (sendEmail && !createdRegistrations.isEmpty()) {
      ...
      StringBuffer sb =
        new StringBuffer("Your request to access the details of the following providers has been granted:\n\n");
