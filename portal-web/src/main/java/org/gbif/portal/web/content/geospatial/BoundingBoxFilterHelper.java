...
public class BoundingBoxFilterHelper implements FilterHelper {
  ...
  public static BoundingBoxDTO getBoundingBoxDTO(String value) {
    ...
    if (elements.size() != 4) {
      logger.debug("bounding box format not reckonized:'" + value
        + "', expected format <min-longitude>E,<min-latitude>S,<max-longitude>W,<max-latitude>N");
      return null;
    }
    return new BoundingBoxDTO(getLatLong(elements.get(0)), getLatLong(elements.get(1)), getLatLong(elements.get(2)),
      getLatLong(elements.get(3)));
  }
   ...
   * @return
   */
  public static String getBoundingBoxQueryString(float minLongitude, float minLatitude, float maxLongitude,
    float maxLatitude) {
  ...
  public static LatLongBoundingBox getLatLongBoundingBox(String value) {
    ...
    return new LatLongBoundingBox(getLatLong(elements.get(0)), getLatLong(elements.get(1)),
      getLatLong(elements.get(2)), getLatLong(elements.get(3)));
  }

  /**
   * @see org.gbif.portal.web.content.filter.FilterHelper#addCriterion2Request(org.gbif.portal.web.filter.CriterionDTO,
  ...
  /**
   * @see org.gbif.portal.web.content.filter.FilterHelper#preProcess(org.gbif.portal.dto.PropertyStoreTripletDTO,
   *      javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
   */
